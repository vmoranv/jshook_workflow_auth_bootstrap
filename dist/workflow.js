import { createWorkflow, SequenceNodeBuilder, ToolNodeBuilder, BranchNodeBuilder, } from '@jshookmcp/extension-sdk/workflow';
...
export default createWorkflow(workflowId, 'Auth Bootstrap')
    ...
    const registerWorkflowId = String(ctx.getConfig(`${prefix}.registerWorkflowId`, 'workflow.register-account-flow.v1'));
    ...
    const preAuthClearUrl = String(ctx.getConfig(`${prefix}.preAuthClearUrl`, registerUrl.replace(/(https?:\/\/[^/]+).*/, '$1/')));
    ...
    const includeConfirmPassword = Boolean(ctx.getConfig(`${prefix}.includeConfirmPassword`, true));
    const confirmPasswordFieldName = String(ctx.getConfig(`${prefix}.confirmPasswordFieldName`, 'checkPassword'));
    const extraFields = ctx.getConfig(`${prefix}.extraFields`, {});
    const checkboxSelectors = ctx.getConfig(`${prefix}.checkboxSelectors`, []);
    const submitSelector = String(ctx.getConfig(`${prefix}.submitSelector`, "button[type='submit']"));
    const registerTimeoutMs = Number(ctx.getConfig(`${prefix}.registerTimeoutMs`, 90_000));
    const enableEmailVerification = Boolean(ctx.getConfig(`${prefix}.enableEmailVerification`, false));
    const emailProviderUrl = String(ctx.getConfig(`${prefix}.emailProviderUrl`, ''));
    const mailOpenWorkflowId = String(ctx.getConfig(`${prefix}.mailOpenWorkflowId`, 'workflow.temp-mail-open-latest.v1'));
    const mailOpenConfig = ctx.getConfig(`${prefix}.mailOpenConfig`, {});
    const mailExtractWorkflowId = String(ctx.getConfig(`${prefix}.mailExtractWorkflowId`, 'workflow.temp-mail-extract-link.v1'));
    const mailExtractConfig = ctx.getConfig(`${prefix}.mailExtractConfig`, {});
    const postVerifyNavigateUrl = String(ctx.getConfig(`${prefix}.postVerifyNavigateUrl`, ''));
    const authExtractPageUrl = String(ctx.getConfig(`${prefix}.authExtractPageUrl`, ''));
    const runAuthExtract = Boolean(ctx.getConfig(`${prefix}.runAuthExtract`, true));
    const runNetworkAuthExtract = Boolean(ctx.getConfig(`${prefix}.runNetworkAuthExtract`, true));
    const authMinConfidence = Number(ctx.getConfig(`${prefix}.authMinConfidence`, 0.3));
    const fields = {
        username,
        email,
        password,
    ...
    const mailboxBranch = new BranchNodeBuilder('maybe-email-verification', 'auth_bootstrap_enable_email_verification');
    mailboxBranch
        .predicateFn(() => enableEmailVerification && Boolean(emailProviderUrl))
        .whenTrue(new SequenceNodeBuilder('email-verification-sequence')
        ...
        .branch('maybe-post-verify-navigate', 'auth_bootstrap_post_verify_navigate', (b) => {
        b.predicateFn(() => Boolean(postVerifyNavigateUrl))
            .whenTrue(new ToolNodeBuilder('navigate-post-verify-page', 'page_navigate').input({
            url: postVerifyNavigateUrl,
            waitUntil: 'networkidle',
            enableNetworkMonitoring: true,
        }))
            .whenFalse(new ToolNodeBuilder('skip-post-verify-navigate', 'console_execute').input({
            expression: '({ skipped: true, step: "post_verify_navigate", reason: "postVerifyNavigateUrl not configured" })',
        }));
    }))
        .whenFalse(new ToolNodeBuilder('skip-email-verification', 'console_execute').input({
        expression: '({ skipped: true, step: "email_verification", reason: "config_disabled" })',
    }));
    // ── Branch: maybe-auth-extract ──
    const authExtractBranch = new BranchNodeBuilder('maybe-auth-extract', 'auth_bootstrap_run_auth_extract');
    authExtractBranch
        .predicateFn(() => runAuthExtract)
        .whenTrue(new SequenceNodeBuilder('auth-extract-sequence')
        .branch('maybe-navigate-auth-extract-page', 'auth_bootstrap_auth_extract_page_url', (b) => {
        b.predicateFn(() => Boolean(authExtractPageUrl))
            .whenTrue(new ToolNodeBuilder('navigate-auth-extract-page', 'page_navigate').input({
            url: authExtractPageUrl,
            waitUntil: 'networkidle',
            enableNetworkMonitoring: true,
        }))
            .whenFalse(new ToolNodeBuilder('skip-auth-extract-page-nav', 'console_execute').input({
    ...
    const networkAuthBranch = new BranchNodeBuilder('maybe-network-auth-extract', 'auth_bootstrap_run_network_auth_extract');
    networkAuthBranch
        .predicateFn(() => runNetworkAuthExtract)
        .whenTrue(new ToolNodeBuilder('network-auth-extract', 'network_extract_auth').input({
        minConfidence: authMinConfidence,
    }))
        .whenFalse(new ToolNodeBuilder('skip-network-auth-extract', 'console_execute').input({
    ...
    root
        .branch('maybe-preclear-auth-state', 'auth_bootstrap_preclear_auth_state', (b) => {
        b.predicateFn(() => clearCookiesFirst || clearStorageFirst)
            .whenTrue(new SequenceNodeBuilder('preclear-auth-state-sequence')
            ...
            .branch('maybe-clear-cookies', 'auth_bootstrap_clear_cookies_first', (cb) => {
            cb.predicateFn(() => clearCookiesFirst)
                .whenTrue(new ToolNodeBuilder('clear-cookies', 'page_clear_cookies').input({}))
                .whenFalse(new ToolNodeBuilder('skip-clear-cookies', 'console_execute').input({
                expression: '({ skipped: true, step: "clear_cookies", reason: "config_disabled" })',
            }));
        })
            .branch('maybe-clear-storage', 'auth_bootstrap_clear_storage_first', (sb) => {
            sb.predicateFn(() => clearStorageFirst)
                .whenTrue(new ToolNodeBuilder('clear-web-storage', 'page_evaluate').input({
                code: '(() => { try { localStorage.clear(); sessionStorage.clear(); } catch {} return { ok: true, href: location.href }; })()',
            }))
                .whenFalse(new ToolNodeBuilder('skip-clear-storage', 'console_execute').input({
                expression: '({ skipped: true, step: "clear_storage", reason: "config_disabled" })',
            }));
        }))
            .whenFalse(new ToolNodeBuilder('skip-preclear-auth-state', 'console_execute').input({
